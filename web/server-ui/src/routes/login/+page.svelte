<script lang="ts">
  ...
  onMount(async () => {
    ...
    try {
      ...
      // If config fails, show local login only
    }
  });
...
<div class="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-950 relative">
  <div class="bg-white dark:bg-slate-800 rounded-lg shadow-lg p-8 max-w-sm w-full text-center">
    <h1 class="text-2xl font-bold text-gray-900 dark:text-white mb-2">Trasker</h1>
    <p class="text-gray-500 dark:text-gray-400 mb-8">Sign in to your organization's time tracking dashboard</p>
    ...
    {#if showAdminLogin}
      ...
      <form onsubmit={(e) => { e.preventDefault(); handleLocalLogin(); }} class="text-left space-y-4">
        ...
        <button
          ...
          class="w-full text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300 text-sm py-1"
        >
          Back
        </button>
      </form>
    {:else}
      ...
      {:else}
        <p class="text-gray-400 dark:text-gray-500 text-sm mb-4">
          Configure Microsoft Entra ID in admin settings to enable organization sign-in.
        </p>
      {/if}
    {/if}
  </div>
  ...
  {#if !showAdminLogin}
    <button
      ...
      class="absolute bottom-4 right-4 text-xs text-gray-300 dark:text-gray-600 hover:text-gray-500 dark:hover:text-gray-400 transition-colors"
    >
      Admin
    </button>
  {/if}
